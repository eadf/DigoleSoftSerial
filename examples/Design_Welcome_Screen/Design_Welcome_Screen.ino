...
#define _DOWNLOAD_WELCOME_;
//#define _DOWNLOAD_MICRO_COMMANDS_;
#define _NEW_FW_;	//the firmware is V2.9 and later
...
void setup() {
  ...
  #else
    for (int j = 2; j < sizeof(welOld);j++) {
         mydisp.write(pgm_read_byte_near(welOld+j));  
    }
  #endif
  ...
  #else
    mydisp.uploadStartScreen(sizeof(welOld),welOld);
  #endif
  //end downloading
#endif
